...
import type { Segment, RouteChoice, BlendedRoute } from '../types/segments';
...
const BlendedRouteContext = createContext<BlendedRouteContextValue | null>(null);
 ...
 */
function encodeSelectionsToUrl(selections: Map<string, RouteChoice>, segments: Segment[]): string {
 ...
 */
function decodeSelectionsFromUrl(param: string, segments: Segment[]): Map<string, RouteChoice> {
...
export function BlendedRouteProvider({ children, segments }: BlendedRouteProviderProps) {
  const [isBuilding, setIsBuilding] = useState(false);
  const [selections, setSelections] = useState<Map<string, RouteChoice>>(new Map());
  const [savedSelections, setSavedSelections] = useState<Map<string, RouteChoice>>(new Map());
  ...
  // Update URL when saved selections change
  useEffect(() => {
    if (savedSelections.size === 0) {
      // Remove route param if no selections
      const params = new URLSearchParams(window.location.search);
      if (params.has('route')) {
        params.delete('route');
  ...
  const exitBuildMode = useCallback((save: boolean) => {
    if (save && isComplete) {
  ...
  const selectSegment = useCallback((segmentId: string, choice: RouteChoice) => {
    setSelections(prev => {
...
export function useBlendedRoute(): BlendedRouteContextValue {
  ...
  if (!context) {
    throw new Error('useBlendedRoute must be used within a BlendedRouteProvider');
  }
  return context;
}
